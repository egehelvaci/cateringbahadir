...
2. VESSEL: Emails about ships/vessels seeking cargo
   - Keywords: vessel, ship, tanker, bulker, dwt, ballast, open, available
   - Phrases: "vessel available", "seeking cargo", "ship open", "vessel positioning"
...
`;
  ...
  private fallbackClassification(subject: string, body: string): EmailClassification {
    ...
    if (cargoScore > vesselScore) {
      return {
        type: 'CARGO',
        confidence: Math.min(cargoScore / cargoKeywords.length, 0.8),
        reason: 'Fallback keyword-based classification detected cargo-related content'
      };
    } else if (vesselScore > cargoScore) {
      return {
        type: 'VESSEL', 
        confidence: Math.min(vesselScore / vesselKeywords.length, 0.8),
        reason: 'Fallback keyword-based classification detected vessel-related content'
      };
    } else {
      return {
        type: 'UNKNOWN',
